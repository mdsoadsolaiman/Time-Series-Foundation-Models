...
def build_notebooks() -> None:
    ...
    specs["02_Bitcoin_Classical_Baselines.ipynb"] = notebook("Bitcoin Classical Baselines", "Artifact-only comparison of final past-only classical systems, with an explicitly historical static-protocol appendix.",
        "Frozen validated forecasts, canonical target, and historical executed static-protocol RMSE values recovered from Git history.", "Final classical metrics, seasonality evidence, and a labelled historical static-vs-fair comparison.", "01 Bitcoin Data/EDA.", "AUTHORITATIVE ANALYSIS",
    ...
    specs["06_Bitcoin_Foundation_Models.ipynb"] = notebook("Bitcoin Foundation Models", "Artifact-first analysis plus safely gated regeneration capability for the two completed zero-shot systems.",
        "Canonical target and frozen Chronos, TimesFM, and validated vectors; optional model checkpoints only when generation is explicitly enabled.", "Artifact-derived comparison by default; when explicitly enabled, candidate forecasts only under `results/staging/bitcoin/<run-id>/`; authoritative replacement only through a separately gated validation-and-promotion step.", "01 and 07.", "AUTHORITATIVE MODEL ANALYSIS / GATED GENERATION",
